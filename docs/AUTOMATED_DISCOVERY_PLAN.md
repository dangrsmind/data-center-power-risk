...
## Build-Constraint Query Expansion

The registry includes a controlled targeted expansion for high-value data center markets and a build-constraint query expansion for the post-pivot intelligence workflow. Targeted entries use official or public domains in generic web-search patterns for Northern Virginia county planning records, Texas utility/regulatory materials, Georgia utility and economic development sources, Ohio/Indiana public sources, North Carolina/South Carolina commerce sources, Arizona/Nevada utility regulators, and Oregon/Washington utility regulators.

The build-constraint entries add coverage for grid and transmission constraints, interconnection, substations, load requests, onsite and behind-the-meter power, dedicated power plants, gas turbines, diesel or backup generators, fuel cells, nuclear/SMR proposals, air permits, emissions, NOx, water use, cooling water, wastewater, drought, public hearings, zoning, land use, moratoria, litigation, community opposition, resident opposition, noise, traffic, regulatory approval, utility commissions, tax incentives, cost/financing, delays, pauses, and cancellations.
...
Recommended pre-live workflow: validate the registry, run the full discovery plan report, save ignored local snapshots for scoped reports such as `--exclude-generic` or `--priority high --exclude-generic --max-planned-queries 30`, compare the snapshots, inspect high-count and overbroad query warnings, run `python scripts/run_public_discovery.py --dry-run`, and only then consider the confirmed capped live command with explicit cost approval.
...
Ingested discovered sources are source/evidence candidates only. Ingestion stores URLs, titles, publisher/geography, discovery method, search term, snippet, case number/document type, registry/adapter context when present, raw metadata, and analyst review metadata. It does not create projects, claims, project links, or promoted evidence. Re-running the ingest is idempotent by `source_url`: duplicate URLs within one discovery output are skipped deterministically, existing database URLs are skipped by default, and safe metadata can be updated with `--allow-existing` without overwriting analyst review status.

Use `GET /discovered-sources`, `GET /discovered-sources/summary`, `PATCH /discovered-sources/{source_id}/review`, `PATCH /discovered-sources/review/bulk`, or the `/discovered-sources` analyst console page to inspect and triage ingested `DiscoveredSourceRecord` rows before claim extraction, candidate generation, verification, auto-admission, or promotion. The review surface supports filtering by run, registry, adapter, source type, geography, source status, publisher, URL quality, weak URL quality, analyst review status, reviewer, review notes presence, computed priority bucket, minimum priority score, and text search over title, URL, snippet, and query. List responses expose bounded provenance, URL-quality fields, computed review-priority fields, and analyst triage fields by default; the single-record endpoint returns full raw metadata for one source.

Review priority is computed heuristic guidance, not truth and not ML. It is derived read-only from existing source fields such as source type, official/regulatory URL host, title/snippet/query signal terms, discovery run provenance, weak URL-quality metadata, generic-looking titles, unknown registry/adapter provenance, and snippet length. The API exposes `review_priority_score`, `review_priority_bucket`, and `review_priority_reasons`; sorting by `priority_desc` creates a read-only review queue for analyst triage. These computed fields are not persisted and do not change source status.

Discovered-source analyst triage is a gate, not a downstream action. Supported review statuses are `unreviewed`, `useful`, `maybe`, `noisy`, `weak`, and `rejected`; existing rows with null `review_status` are treated as `unreviewed`. Single-row and bulk triage update only `review_status`, `review_notes`, `reviewed_by`, and `reviewed_at`; bulk triage can replace or append notes for up to 200 selected rows and reports missing source IDs instead of silently ignoring them. Triage does not create Projects, Evidence, claims, ProjectCandidates, verification results, auto-admission decisions, or promotions.

Analyst triage remains authoritative and is separate from computed review priority. The priority queue can suggest that official filings, planning records, economic development notices, or project-like snippets deserve early review, but only analyst `review_status` records the triage decision.

Analyst triage is also separate from machine URL-quality warnings. SCC public-comment form and fallback-reference URL-quality warnings are provenance warnings, not analyst rejection and not structural failures. A weak SCC URL can still be marked `useful` or `maybe` when an analyst wants to keep it as fallback context, and a primary-looking URL can still be marked `noisy` or `rejected` if it is not useful. Downstream extraction should later default to reviewed `useful` or `maybe` sources, with high-signal reviewed sources first when appropriate, unless an operator explicitly overrides that gate. Extraction is not part of the review-priority queue branch.
