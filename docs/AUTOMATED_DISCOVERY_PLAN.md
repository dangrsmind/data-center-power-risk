...
Discovered-source analyst triage is a gate, not a downstream action. Supported review statuses are `unreviewed`, `useful`, `maybe`, `noisy`, `weak`, and `rejected`; existing rows with null `review_status` are treated as `unreviewed`. Triage updates only `review_status`, `review_notes`, `reviewed_by`, and `reviewed_at`. It does not create Projects, Evidence, claims, ProjectCandidates, verification results, auto-admission decisions, or promotions.
