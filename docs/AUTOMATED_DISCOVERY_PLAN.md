...
Use `GET /discovered-sources`, `GET /discovered-sources/summary`, `PATCH /discovered-sources/{source_id}/review`, or the `/discovered-sources` analyst console page to inspect and triage ingested `DiscoveredSourceRecord` rows before claim extraction, candidate generation, verification, auto-admission, or promotion. The review surface supports filtering by run, registry, adapter, source type, geography, source status, publisher, URL quality, weak URL quality, analyst review status, reviewer, review notes presence, and text search over title, URL, snippet, and query. List responses expose bounded provenance, URL-quality fields, and analyst triage fields by default; the single-record endpoint returns full raw metadata for one source.
...
Analyst triage is separate from machine URL-quality warnings. SCC public-comment form and fallback-reference URL-quality warnings are provenance warnings, not analyst rejection and not structural failures. A weak SCC URL can still be marked `useful` or `maybe` when an analyst wants to keep it as fallback context, and a primary-looking URL can still be marked `noisy` or `rejected` if it is not useful. Downstream extraction should later be scoped to `useful` and `maybe` discovered sources by default unless an operator explicitly overrides that gate.
